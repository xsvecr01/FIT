...
*/
	...
	//kontrola jestli je s == NULL, pokud ano vola se funcke stackError
	if(s == NULL)
	{
...
*/
	...
	//pokud s->top je rovno -1, vraci 1, jinak 0
	return s->top == -1;
...
*/
	...
	//pokud je s->top rovno STACK_SIZE-1 vraci se 1
...
*/
	...
	//pokud je zasobnik prazdny, vola se fce stackError
	if (stackEmpty(s) == 1)
	{
		stackError(SERR_TOP);
	}

	//jinak se priradi do ukazatele c znak z vrcholu zasobniku
	else
	{
		*c = s->arr[s->top];
	}
...
** funkci stackFull.
*/
	//kontrola, jestli je zasobnik plny
	if(stackFull(s) == 1)
	{
		stackError(SERR_PUSH);
	}

	//zvyseni hodnoty top a pridani prvku c
	else
	{
		s->top++;
		s->arr[s->top] = c;
